...
PAPER_CONTEXT_V4 = (
    "EVALUATION CONTEXT — READ BEFORE SCORING (v4.2):\n\n"
    ...
    "  (2) a deep-dive case on the carpool module, grounded in a formative survey "
    "with N=111 eligible respondents and N=44 who finished the full instrument "
    "(was 117/50 in v4.1; the 6 Survey Preview rows were excluded as researcher "
    "test responses);\n"
    ...
    "are NOT required.\n"
    "- EVIDENCE BAR: N=111 eligible / 44 finished formative survey (per-item N "
    "typically 30-33 on ordinal items) is a reasonable formative-stage sample. "
    "Do NOT apply inferential-statistics standards. No t-tests or p-values are "
    "expected or appropriate. Every statistic is expected to report its per-item N.\n"
    "- SAMPLE SKEW: 79% Mandarin-native respondents (72/91 of those who reported "
    "native language; was 82% in v4.1) is DISCLOSED as a scope-setting property. "
    "Evaluate whether the paper scopes its claims accordingly — NOT whether the "
    "sample is demographically balanced.\n"
    ...
    "FINDING F5 (v4.2 subset revision): Beat 5's rating-fairness asymmetry is "
    "reported on the Driver/Both subset (N=19) with a Rider-only control subset "
    "(N=12). The Driver/Both subset shows an unfair-rating tolerance of 29.1 "
    "versus 41.4-52.3 for the other three tolerance items (late passenger 47.2, "
    "destination change 41.4, non-standard route 52.3); the Rider-only control "
    "shows a markedly weaker pattern (unfair rating 22.4 versus 19.6-35.7 on "
    "the others). This subset methodology is a DELIBERATE CHOICE — rider "
    "respondents answer the driver-tolerance questions hypothetically, so "
    "mixing them with real drivers would dilute the signal. Any Beat 5 "
    "narrative that uses the full-sample N=30 is an outdated v4.1 artifact "
    "and should be flagged as a correctness issue, not rewarded as broader "
    "coverage.\n\n"
    "FINDING F6 (v4.2 new): Q23 driver supply willingness across 33 respondents "
    "shows long-distance trips (12/33 Very+Extremely willing) as the highest "
    "willingness segment, ahead of in-Ithaca (10/33) and short-distance (9/33). "
    "This directly informs the carpool module's long-distance scope choice in "
    "§5.2 and should be cited as design motivation rather than deployment "
    "validation.\n"
)
...
REVIEWER_PROMPTS = {
    "narrative": (
        "You are the Narrative Reviewer for the v4.2 CampusRide paper. Evaluate whether the 7-beat "
        ...
        "survey passenger and driver-subset sides), a core-contribution artifact beat (6 = CampusRide "
        "platform with carpool deep-dive), and an adversarial scoping beat (7 = algorithmic "
        "management critique + three-tier sample skew + no deployment). "
        ...
        "v4.2 CORRECTNESS CHECK: Beat 5 narrative MUST discuss the Driver/Both subset (N=19) with "
        "the Rider-only control (N=12); any Beat 5 narrative that uses full-sample N=30 is outdated "
        "(v4.1 artifact) and should be flagged as a correctness issue, not treated as additional "
        "coverage. The subset-level framing is a deliberate methodological choice. "
        "Score based on: "
    ...
    "contradiction": (
        "You are the Contradiction Reviewer for the v4.2 CampusRide paper. Evaluate whether the "
        ...
        "v4.2 EXPECTED CROSS-CATEGORY CONTRADICTION: H-subgroup literature (amateur / P2P carpool "
        "rating) vs professional gig-worker algorithmic management is an expected cross-category "
        "contradiction for v4.2. Surface it under the F4 Rating Fairness focus and treat it as a "
        "legitimate scope tension rather than flagging it as reviewer noise. "
    ...
    "gap": (
        "You are the Gap Reviewer for the v4.2 CampusRide paper. Evaluate whether the gap / "
        ...
        "(b) Beat 5 F5 driver-subset finding is scope-limited to N=19 (Driver/Both subset with "
        "N=12 Rider-only control). Do NOT inflate this to full-sample generalization, and do NOT "
        "over-generalize it to all peer rating systems, "
    ...
    "coverage": (
        "You are the Coverage Checker for the v4.2 CampusRide paper. Evaluate coverage against the "
        ...
        "v4.2 CATEGORY TARGETS (unchanged from v4.1 CATEGORY_TARGETS dict): A:8, B:10, C:8, D:8, "
        "E:8, F:10, G:8, H:10, I:8, J:10. Highest-priority active targets remain A:8, B:10, F:10, "
        "H:10, J:10; C:8, D:8, I:8 are thin coverage targets that may need Lens supplement; "
        "E:8, G:8 should remain strong. "
        "CRITICAL v4.2 H-SUBGROUP RULE: H-subgroup secondary label is EXPECTED. If papers carry "
        "secondary_categories containing 'H-subgroup' or 'H-amateur', count them as H coverage "
        "AND as subgroup-specific coverage for Beat 5. Do not treat the secondary label as "
        "double counting or as category drift. "
    ...
    "honesty": (
        "You are the Honesty Reviewer for the v4.2 CampusRide paper. Check whether the final "
        "evidence outputs stay within what the corpus and the N=111 eligible / 44 finished "
        "formative survey actually support. v4.2 beat-wise checklist (aligned with Part 6 verb "
        "discipline table of paper_outline_v4.2.md): "
        "(1) Beat 1: frame the small-town transport gap as motivated / documented, NOT at-scale "
        "empirically quantified. Updated F1 numbers are 28/32 (Uber pricing) and 23/32 "
        "(availability). If the draft still uses v4.1 numbers 29/33 and 24/33, flag it as a "
        "correctness / honesty violation. "
        "(2) Beat 2: grassroots coordination is documented; super-app framing must be TIERED "
        "based on Prompt B I-category audit: N<3 = 'initial inquiry with fragmented precedent'; "
        "N=3-4 = 'emerging design space with several case studies'; N>=5 = 'established design "
        "space with documented value propositions'. Do NOT claim CampusRide is the 'first "
        "multi-module campus platform'. "
        "(3) Beat 3: four design primitives are presented as parallel, NOT value-ranked. The "
        "Identity primitive is TIERED on Prompt A E-category audit output: N<3 = "
        "'under-investigated, case treatment'; N=3-4 = 'discussed case-by-case, less "
        "systematic'; N>=5 = 'mature literature specialized to institutional scoping'. Flag "
        "any upward drift beyond the audit-supported tier. "
        "(4) Beats 4-5: descriptive statistics only — NO inferential stats, no 'significantly', "
        "no t-tests / p-values. The 79% Mandarin-native cohort (72/91) must be explicitly "
        "disclosed as scope, not masked as bias. Every statistic must report its per-item N. "
        "F5 must be attributed to the Driver/Both subset (N=19) with Rider-only (N=12) control "
        "numbers reported (unfair rating 29.1 vs 22.4; other items 41.4-52.3 vs 19.6-35.7). "
        "F6 (v4.2 new) must be added for Q23 passenger-supply willingness with long-distance "
        "12/33 reported as the highest willingness segment. "
        "(5) Beat 6: platform is DESIGNED and MOTIVATED, not validated. Use 'we designed' / "
        "'motivated by' / 'in response to finding X'; NEVER use 'effective', 'successful', "
        "'proves', 'validates'. F5 attribution in §5.8.3 MUST be at the Driver/Both subset "
        "level (N=19), not full sample. "
        "(6) §6.2 Audit Trail: MUST contain a negative audit paragraph (¶3) that explicitly "
        "discloses pipeline shortcomings (e.g., E-category proxy mislabeling pre-audit; no "
        "automatic subset sensitivity check on primary-data findings). Flag any §6.2 text that "
        "describes the pipeline without admitting its limits. "
        "(7) Beat 7: THREE-TIER sample skew disclosure is mandatory and the tiers must appear "
        "SEPARATELY, not merged: (i) main sample 79% Mandarin-native bias; (ii) driver subset "
        "N=19; (iii) completion rate ~40% (44/111). Each adversarial paragraph must cite at "
        "least one counterevidence paper and use 'may reproduce' / 'we acknowledge' / "
        "'scope-limited'. Flag any 'we address' / 'we prevent' / 'we solve'. "
