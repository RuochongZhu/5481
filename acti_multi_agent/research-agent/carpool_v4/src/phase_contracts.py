...
v4 relaxation: beats marked as primary_data or artifact (e.g. Beat 4 formative
survey, Beat 6 CampusRide system) may use a pseudo-anchor paperId prefixed
with LOCAL_ANCHOR_PREFIX. Their spine may reference internal section or
finding identifiers as strings rather than real paperIds.
"""
...
# Load beat type annotations from config so primary_data / artifact beats can
# be validated leniently. Import-style mirrors scoring.py.
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))
try:
    from config.beat_definitions import BEAT_TYPES, LOCAL_ANCHOR_PREFIX
except ImportError:  # pragma: no cover - safe fallback for legacy configs
    BEAT_TYPES = {}
...
def ensure_narrative_chains_valid(chains: list[dict]):
    ...
    for idx, chain in enumerate(chains):
        ...
        valid_beats += 1
        beat_num = chain.get("beat")
        beat_type = BEAT_TYPES.get(beat_num)
        is_primary_shim = beat_type in {"primary_data", "artifact"}

        anchor = chain.get("anchor_paper")
        anchor_paper_id = anchor.get("paperId") if isinstance(anchor, dict) else None

        if not isinstance(anchor, dict) or not _is_nonempty_str(anchor_paper_id):
            errors.append(f"beat {chain.get('beat', '?')} missing valid anchor_paper.paperId")
        elif not is_primary_shim and isinstance(anchor_paper_id, str) and anchor_paper_id.startswith(LOCAL_ANCHOR_PREFIX):
            # Only primary_data / artifact beats may use local pseudo-anchors.
            errors.append(
                f"beat {chain.get('beat', '?')} uses pseudo-anchor '{anchor_paper_id}' "
                f"but is not declared as primary_data/artifact"
            )
